...
#### E. DLS (Dynamic Level Scheduling)
1. Berechne für jeden bereiten Task $n_i$ auf jedem Prozessor $p_j$ den **Dynamic Level**:
$$DL(n_i, p_j) = b\text{-level}(n_i) - EST(n_i, p_j)$$
2. Wähle das Paar $(n_i, p_j)$ mit dem größten DL-Wert.
* **Idee:** Der Dynamic Level kombiniert die statische Kritikalität (b-level) mit der aktuellen Auslastungssituation (EST). Ein hoher DL bedeutet: wichtiger Task, der bald starten kann.
* **Vorteil:** Berücksichtigt dynamisch sowohl Abhängigkeiten als auch Kommunikationskosten.

#### F. LAST (Localized Allocation of Static Tasks)
1. Berechne den **relativen Mobilitätswert** jedes Tasks: Differenz zwischen ALAP- und frühestmöglicher Startzeit. Tasks mit geringer Mobilität sind weniger flexibel und sollten zuerst eingeplant werden.
2. Weise Tasks mit geringster Mobilität zuerst zu — jedoch wird der Task dem Prozessor zugeteilt, der die geringsten Gesamtkosten (Kommunikation + Wartezeit) erzeugt.
* **Schwäche:** Laut empirischer Studie schneidet LAST unter allen BNP-Algorithmen am schlechtesten ab.
